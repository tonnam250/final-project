        ...
        <div className="flex flex-col text-center w-full justify-center items-center h-full pt-20">
          ...
          <div className="flex h-full w-11/12 md:w-8/12 h-11/12 p-4 md:p-5 shadow-xl justify-center items-center border rounded-2xl m-2 md:m-5">
            <form
              ...
              onSubmit={isCreating ? handleCreateFactory : handleUpdateFactory}
            >
      
            {/* Factory Name */}
      <div className="flex flex-col text-start w-full">
        <label htmlFor="factoryName" className="font-medium">Company Name</label>
        <input
          type="text"
          id="factoryName"
          name="factoryName"
        ...
        onClick={async (event) => {
          ...
          if (isEditable) {
            if (isCreating) {
              await handleCreateFactory(event); // ถ้าอยู่ในโหมด Create → ใช้ function เดิม
            } else {
              await handleUpdateFactory(event); // ถ้าอยู่ใน Edit Mode → Update
