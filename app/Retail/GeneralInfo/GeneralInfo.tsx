...
const handleUpdateRetailer = async (event: React.FormEvent) => {
    ...
    return (
        ...
        <h1 className="text-3xl md:text-4xl font-bold my-4 md:my-8">{isCreating ? "Create Your Retailer" : "General Information"}</h1>
        ...
        <form className="flex flex-col gap-4 w-full" onSubmit={isCreating ? handleCreateRetailer : handleUpdateRetailer}>

{/* Retailer Name */}
<div className="flex flex-col text-start w-full">
    <label htmlFor="retailerName" className="font-medium">Retailer Name</label>
    <input
        type="text"
        id="retailerName"
        name="retailerName"
    ...
    />
                    ...
                    <div className="flex flex-col text-start font-medium">
                        ...
                        <textarea name="address" id="address" className="border border-gray-300 rounded-md p-2 flex-1 w-full" disabled={!isCreating && !isEditable}value={retailerData?.address || ""}onChange={handleInputChange}></textarea>
                    </div>
                    {/* end Address */}
...
{!isCreating && (
    <div className="flex flex-col border p-2">
        <h3 className="font-medium">Retailer Certificates</h3>
...
<button
    ...
    onClick={async (event) => {
        ...
        if (isCreating) {
            await handleCreateRetailer(event); // ✅ เรียกฟังก์ชันสร้างร้านค้าโดยตรง
        } else if (isEditable) {
            await handleUpdateRetailer(event); // ✅ เรียกฟังก์ชันอัปเดตร้านค้าโดยตรง
            setIsEditable(false);
        } else {
...
>
    {isCreating ? "Create Retailer" : isEditable ? "Save" : "Edit"}
    {isCreating ? (
