    ...
    return (
        <div className="flex flex-col md:flex-row px-4 md:px-8 h-18 w-full bg-[#6C0E23] items-center text-[#FFFFD0] text-xl md:text-2xl font-semibold fixed z-10">
            ...
            <div className={`flex-col md:flex-row md:flex gap-4 md:gap-24 justify-end items-center w-full md:w-1/2 ${isMenuOpen ? 'flex' : 'hidden'} md:flex`}>
                <Link href={'/Retail/GeneralInfo'} className={`hover:text-[#C98986] whitespace-nowrap ${pathname === "/Retail/GeneralInfo" ? "text-[#C98986]" : ""}`}>General Information</Link>
                <Link href={'/Retail/Recieved'} className={`hover:text-[#C98986] ${pathname === "/Retail/Recieve" ? "text-[#C98986]" : ""}`}>Recieved</Link>
                <Link href={'/Retail/Delivering'} className={`hover:text-[#C98986]  ${pathname === "/Retail/Deliver" ? "text-[#C98986]" : ""}`}>Delivering</Link>
                <Link href={'/Retail/Profile'} className={`flex items-center border-2 border-[#EFE4DC] rounded-full px-2 md:px-3 py-1 md:py-2 shadow-md gap-2 md:gap-3 cursor-pointer hover:text-[#7B9B9E] hover:bg-[#E1EABB]`}>
